#################################################File_Name : MRO ANALYTICS ########################################################################################################################
##File_Owner : Renjiith Sebastian###########################################################################################################################################################################
## Description : Code for Creation Analytic Record for Equipment -EAR--And Load Into Datawarehouse
###############################################Source Data###############################################################################################################
#####***WellView:: TABLES[TUBING,TUBING_COMPONENTS,PROBLEM,PROBLEM_DETAILS,WELLVIEW WELLHEADER]##########################################################################
#####***PROCOUNT:: TABLES[PROCOUNT WELL HEADER,PROCOUNT DAILYWELL DATA]##########################################################################################################
#####***ESP:::::::
###############################################Join Conditions################################################################################################################################################
#####*** (TUBONG.IDREC=TUBING_COMPONENTS.IDRECPARENT)***TUBING_DATA
#####*** (PROBLEM.IDWELL=PROBLEM_DETAILS.IDWELL PROBLEM.IDREC=PROBLEM_DETAILS.IDRECPARENT)***PROBLEM_DATA
############################################################################################################################################################################################
#####*** ((((((((((TUBING_DATA.IDWELL=PROBLEM_DATA.PBM_IDWELL AND TUBING_DATA_COMP.IDREC=PROBLEM_DATA.PBM_IDREC
#####**************AND PROBLEM_DATA.DTTMSTART_ASDATE>=TUBING_DATA.DTTMRUN_ASDATE***************************
#####**************AND TUBING_DATA.DTTMSTART_ASDATE<=TUBING_DATA.DTTMPULL_ASDATE)))))))********************
###################################################################################################################################################################################################
## Created Date : 2020/02/21(YYYYMMDD)###############################################################################################################################################################
...
PCDAILYWELLDATA_FILE = "pct-wellproductionvolumes-sixtyday"
...
try:
    ...
    print("Error: Check All the Sources files Available in respective path ")
else:
    print('Execute Job')
...
df_esp = spark.read.parquet("s3://mro-dev-esp-analytics-bucket/MRO_Analytics/DataSources/ESP_Processed/Active/")
...
df_esp_1=spark.sql("""SELECT
                         ...
                         --production_month
                            ...
                            where TO_DATE(PRODUCTION_DATE)>=date_add(current_date(), -60)
                            """)
...
df_EAR_dynamicFrame = DynamicFrame.fromDF(df_EAR, glueContext, "df_EAR_dynamicFrame")
 ...
 # Write the transformed dataset to S3 with Partitioning
datasink5 = glueContext.write_dynamic_frame.from_options(frame = df_EAR_dynamicFrame, connection_type = "s3", connection_options = {"path": "s3://mro-dev-esp-analytics-bucket/MRO_Analytics/DataOutputs/Snowflake_Outbound/EAR/", "partitionKeys": ["LOAD_DATE"]}, format = "parquet", transformation_ctx = "datasink5")
